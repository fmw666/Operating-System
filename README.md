...
+ 在传统的多道程序环境下，要使作业运行，必须先为它创建一个或多个进程，并为之分配必要的资源。当进程运行结束时，立即撤销该进程，以便能及时回收该进程所占用的各类资源。

+ 主要功能是为作业创建进程、撤销已结束的进程，以及控制进程在运行过程中的状态转换。

+ 在现代OS中，进程控制还应具有为一个进程创建若干个线程的功能和撤销（终止）已完成任务的线程的功能。 
